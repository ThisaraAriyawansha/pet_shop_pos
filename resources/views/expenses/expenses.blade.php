  ...
  <div class="h-5/6 max-lg:h-[83vh] flex flex-col items-center">
    ...
    <div class="h-full w-fit">
      ...
      <div
        ...
        <a href="{{ asset('expenses/addExpense')}}">
        <div 
          ...
          <div class="w-10 h-10" style="
                background: url('{{ asset('images/expenses/addNewExpense.png') }}')
                  no-repeat;
                background-size: cover;
              "></div>
          <p class="text-center max-sm:text-sm">Add New Expense</p>
        </div>
        </a>
        @endif
        
        @if(has_permission(71))
        <a href="{{ asset('expenses/addExpenseCategory')}}">
        <div 
          class="w-[200px] max-lg:w-[150px]  border-2 border-[{{ $settings[7]->value}}] h-[200px] max-lg:h-[150px]  bg-[{{ $settings[7]->value}}] text-white rounded-lg flex flex-col gap-3 justify-center items-center hover:scale-90 transition-all cursor-pointer">
          <div class="w-10 h-10" style="
                background: url('{{ asset('images/expenses/addNewExoenseCategory.png') }}')
                  no-repeat;
                background-size: cover;
              "></div>
          <p class="text-center max-sm:text-sm">Add New Expense Category</p>
        </div>
        </a>
        @endif
        ...
        <div 
          class="w-[200px] max-lg:w-[150px]  border-2 border-[{{ $settings[7]->value}}] h-[200px] max-lg:h-[150px]  bg-[{{ $settings[7]->value}}] text-white rounded-lg flex flex-col gap-3 justify-center items-center hover:scale-90 transition-all cursor-pointer">
          <div class="w-10 h-10" style="
                background: url('{{ asset('images/expenses/ExpenseList.png') }}')
                  no-repeat;
                background-size: cover;
              "></div>
          <p class="text-center max-sm:text-sm">Expenses List</p>
        </div>
        </a>
        @endif

        @if(has_permission(73))
        <a href="{{ asset('expenses/expenseCategoryList')}}">
        <div 
          class="w-[200px] max-lg:w-[150px]  border-2 border-[{{ $settings[7]->value}}] h-[200px] max-lg:h-[150px]  bg-[{{ $settings[7]->value}}] text-white rounded-lg flex flex-col gap-3 justify-center items-center hover:scale-90 transition-all cursor-pointer">
          <div class="w-10 h-10" style="
                background: url('{{ asset('images/expenses/Expenses CategoryList.png') }}')
                  no-repeat;
                background-size: contain;
                background-position: center;
              "></div>
          <p class="text-center max-sm:text-sm">Expenses Category List</p>
        </div>
        </a>
        @endif
      </div>
